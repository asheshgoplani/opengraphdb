...
use ogdb_node::NodeBindingDatabase;
...
fn unique_path(tag: &str) -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .expect("system clock")
        .as_nanos();
    std::env::temp_dir().join(format!("ogdb-node-{tag}-{nanos}.ogdb"))
}

#[test]
fn node_binding_database_crud_query_and_metrics() {
    let path = unique_path("crud");
    let mut db = NodeBindingDatabase::init(path.to_string_lossy().as_ref()).expect("init db");
...
#[test]
fn node_binding_database_index_and_search_helpers() {
...
#[test]
fn node_binding_database_import_and_export_json() {
  ...
  "edges": [
    ...
    let exported = std::fs::read_to_string(export_path).expect("read export payload");
