...
- [x] **Phase 1: Bugfix Verification** - Verify and release the 15 implemented bugfixes that appear in the unreleased CHANGELOG section
- [x] **Phase 2: Type System Completion** - Add temporal (date, datetime, duration) and collection (list, map) property types throughout the engine
- [x] **Phase 3: Operational Capabilities** - Deliver auto-indexing, atomic bulk import, schema migration, and stable embedded API
- [x] **Phase 4: Query Optimization** - Implement worst-case optimal joins and factorized intermediate results for multi-pattern queries
- [x] **Phase 5: Independent Extensions** - Add temporal versioning compaction and SHACL shape validation
- [x] **Phase 6: Quality Validation** - Validate memory and disk size budgets against the 1M-node 5M-edge target
...
Plans:
- [x] 01-01: Audit and write regression tests for BUG-01 through BUG-09 (query engine and parser bugs)
- [x] 01-02: Audit and write regression tests for BUG-10 through BUG-15 (CLI bugs)
- [x] 01-03: Run full test suite, confirm all 15 bugs are green, move CHANGELOG entries to released section
...
Plans:
- [x] 02-01-PLAN.md: Add Date and DateTime property types (PropertyValue variants, custom serde, ISO 8601 parsing, comparison, evaluator wiring)
- [x] 02-02-PLAN.md: Add Duration property type (PropertyValue variant, ISO 8601 duration parsing, duration arithmetic in BinaryOp)
- [x] 02-03-PLAN.md: Add List property type (PropertyValue variant, Subscript AST node, parser postfix [i]/[i..j], ListLiteral rewrite, ListComprehension evaluator)
- [x] 02-04-PLAN.md: Add Map property type (PropertyValue variant, MapProjection AST node, PropertyAccess on maps, MapLiteral rewrite, map projection)
...
Plans:
- [x] 03-01: Implement auto-index creation based on query frequency threshold
- [x] 03-02: Implement all-or-nothing bulk import mode with transactional rollback
- [x] 03-03: Implement `migrate` CLI command with dry-run support
- [x] 03-04: Stabilize Rust embedded API (`db.query()`, `db.query_profiled()`) with documented public types
...
Plans:
- [x] 04-01: Implement WCOJ strategy in the query planner with cardinality-based selection
- [x] 04-02: Implement factorized intermediate result representation in the execution engine
...
Plans:
- [x] 05-01-PLAN.md — Temporal versioning: TemporalNodeVersion data model, add/query/compact APIs, persistence, background compaction, 1000-version compaction test
- [x] 05-02-PLAN.md — SHACL validation: shapes parser via oxrdfio, validate-against-shacl engine, validate-shacl CLI command, conformance and violation tests
...
Plans:
- [x] 06-01: Implement memory budget benchmark and gate assertion (1M nodes + 5M edges < 500MB RSS)
- [x] 06-02: Implement disk budget benchmark and gate assertion (1M nodes + 5M edges < 1GB on-disk)
...
|-------|----------------|--------|-----------|
| 1. Bugfix Verification | 3/3 | Complete | 2026-02-27 |
| 2. Type System Completion | 4/4 | Complete | 2026-02-27 |
| 3. Operational Capabilities | 4/4 | Complete | 2026-02-27 |
| 4. Query Optimization | 2/2 | Complete | 2026-02-27 |
| 5. Independent Extensions | 2/2 | Complete | 2026-02-27 |
| 6. Quality Validation | 2/2 | Complete | 2026-02-27 |
