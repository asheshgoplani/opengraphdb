...
| Full-text indexes | ✅ | `db.index.fulltext.queryNodes(idx, q [, k])` |
| Vector indexes | ✅ | `db.index.vector.queryNodes(idx, vec, k [, metric])` |
| GraphRAG (vector + graph hop in one query) | ✅ | `db.index.hybrid.queryNodes(...)` or HTTP `/rag/search`. First-class; Neo4j needs APOC + custom. |
| Time-travel / bitemporal queries | ✅ as `AT TIME` (ms) + MCP `temporal_diff` (s) | Not in Neo4j Community. |
| RDF import / export | ✅ via CLI (`ogdb import-rdf` / `export-rdf`) and MCP (`import_rdf` / `export_rdf`) | Built-in. Neo4j needs neosemantics plugin. |
...
| APOC procedures | ⚠️ partial → `db.*` or plain Cypher | See mapping below. |
...
| `apoc.text.fuzzyMatch` | `db.index.fulltext.queryNodes(idx, 'q~')` (Lucene-style) |
...
- Vector + full-text + graph in one query (`db.index.hybrid.queryNodes` or HTTP `/rag/search`).
...
- Browser UI. The `ogdb` binary itself is headless; pair with the
  separate `ogdb-frontend` SPA, or use `ogdb shell` for interactive
  exploration.
