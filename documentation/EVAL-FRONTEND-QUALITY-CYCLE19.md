## EVAL · FRONTEND-QUALITY · Cycle 19

**Base commit:** `6c17c3d` (origin/main, post cycle-18 3-commit closure).
**Worktree:** a fresh detached worktree off `origin/main`.
**Scope:** `frontend/`.
**Date:** 2026-05-05.
**Cycle status (per prompt):** cycle-16 + cycle-17 + cycle-18 ALL `0 BLOCKER + 0 HIGH` → 3rd-round CONVERGED. Cycle-19 is **bookkeeping confirmation**.

---

## Frontend churn between cycle-18 base (`91ee552`) and cycle-19 base (`6c17c3d`)

```
$ git log --oneline 91ee552..6c17c3d -- frontend/
(no output — zero frontend files touched)

$ git diff --stat 91ee552..6c17c3d -- frontend/
(empty)
```

The three commits since cycle-18 (`1957b55`, `28b49b6`, `6c17c3d`) only touch `README.md`, `documentation/QUICKSTART.md`, `documentation/BENCHMARKS.md`, `scripts/install.sh`, and skill/migration text. Cycle-18 fixes touched no frontend files (cycle-17's `b5d10c9` already moved README simplification + QUICKSTART addition into `documentation/`, leaving `frontend/` untouched). All gate counts and bundle hashes match cycle-18 / cycle-17 / cycle-16 exactly.

---

## Gate runs (cycle-19, per crate cap, no `--workspace test`)

| Gate | Command | Result | Δ vs cycle-18 |
| --- | --- | --- | --- |
...
| App build | `npm run build:app` | **succeeds** — bundle hashes byte-identical to cycle-18 | unchanged |
...
All eleven gates clean. Cycle-19 reproduces cycle-18 to the test/byte.

---

## Bundle status (cycle-15 F01/F02 KNOWN-TRADEOFF — no growth)

| Chunk | gzip (`.gz` bytes) | brotli | hash vs cycle-18 / cycle-17 / cycle-16 | Realistic 1.6 MB cap |
| --- | --- | --- | --- | --- |
...
### F02 · MEDIUM · React 19 `forwardRef` drift across shadcn-ui *(carryover from cycle-18 F02 / cycle-17 F02 / cycle-16 F02 / cycle-15 F05 — UNFIXED)*

- **Files (8 files still using `React.forwardRef` / destructured `forwardRef`, verified at `6c17c3d` via `grep -rln forwardRef src/components/ui/`):**
  `frontend/src/components/ui/button.tsx`, `card.tsx`, `dialog.tsx`, `table.tsx`, `input.tsx`, `sheet.tsx`, `accordion.tsx`, `motion.tsx`. (Same 8 files as cycle-18; total call-site count unchanged.)
...
### F03 · LOW · Knip surfaces 31 unused exports + 34 unused exported types *(carryover from cycle-18 F03 / cycle-17 F03 / cycle-16 F03 / cycle-15 F06 — UNFIXED)*

- **Detected by:** `npx knip --no-config-hints` (in `frontend/`) — counts `Unused exports (31)` and `Unused exported types (34)`, exact match vs cycle-18, cycle-17, cycle-16.
...
### F04 · LOW · `deck.gl` umbrella package declared but never imported directly *(carryover from cycle-18 F04 / cycle-17 F04 / cycle-16 F04 / cycle-15 F07 — UNFIXED)*
...
### F05 · LOW · `pngjs` + `@types/pngjs` declared but never imported *(carryover from cycle-18 F05 / cycle-17 F05 / cycle-16 F05 — UNFIXED)*
...
- Playwright e2e: **251 tests / 70 files** — meets/exceeds cycle-15 floor (250/70). Net 0 spec drift since cycle-16. (`find frontend/e2e -name "*.spec.ts"` shows 71 files on disk; one is a `mobile-chrome`-only spec that is listed under the mobile project — total listed across all projects is 70 files and 251 tests, matching cycle-18 exactly.)
...
- a11y-axe sweep: covers `/`, `/playground`, `/claims`, `/docs/llm-to-cypher` with WCAG 2 A/AA tags and fails on critical/serious. Coverage matches cycle-18.
...
- Bundle realistic-cap (1.6 MB gzip): both worst-offenders (`cypher-grammar-vendor`, `lintWorker`) at ~1,544.62 kB / ~1,568.45 kB gzip — both **under** the 1.6 MB realistic cap, both byte-identical to cycle-18.
- Doc / install / skill commits since cycle-18 (3 commits: `1957b55` README+QUICKSTART syntax fix, `28b49b6` BENCHMARKS tone-down propagation, `6c17c3d` install.sh path alignment): touched only docs + scripts — frontend gates re-run on `6c17c3d` show identical counts (251/70 specs, 116 vitest, 99 node:test, 0 lint, 0 tsc, identical bundle hashes), confirming the cycle-18 closure leaked no frontend regression.
...
Cycle-15 F01/F02 (BLOCKER bundle-cap violations under the historic 1.5 MB threshold) remain present but stay explicitly KNOWN-TRADEOFF — sizes have not grown (~1,544.62 kB / ~1,568.45 kB gzip, byte-identical content-hashes vs cycle-18). Both are **under the 1.6 MB realistic cap**. All five non-BLOCKER findings are pure carryovers from cycle-15→18; cycle-19 surfaces no new frontend issue.

Per the cycle-19 prompt, **`0 BLOCKER + 0 HIGH` × 4 consecutive cycles (16, 17, 18, 19) = 4th-round CONVERGED**. Bookkeeping confirmation complete.
